...
let lowerList = simpleList.map(fruit => fruit.toLowerCase()).sort();
console.log(lowerList);

const nums = [10, 12, 9, 8, 1];
...
function search() {
    ...
    let filteredHikes = hikes.filter(function(hike){
        return (hike.name.toLowerCase().includes(hikeQuery.toLowerCase())) || (hike.description.toLowerCase().includes(hikeQuery.toLowerCase())) || (hike.tags.find(tag => tag.toLowerCase().includes(hikeQuery)));
    });
